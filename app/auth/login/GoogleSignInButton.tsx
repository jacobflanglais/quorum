  ...
  const handleClick = () => {
    ...
    startTransition(async () => {
      try {
        ...
        // `redirect()` in a server action throws a NEXT_REDIRECT — let
        // Next.js handle it. Any other throw is a real failure.
  ...
  return (
    <div className="flex w-full flex-col gap-3">
      <button
        ...
        onClick={handleClick}
        disabled={isPending}
        className="group inline-flex h-11 w-full items-center justify-center gap-3 rounded-md border border-border bg-surface px-5 text-sm font-medium text-foreground transition-colors hover:bg-surface-elevated hover:border-border-strong disabled:opacity-60"
      >
        <GoogleGlyph />
